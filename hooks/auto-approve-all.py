...
GUARDS = {
    ...
    # Restores the subset invariant for dispatch: without this, auto-approve
    # answers `allow` for a catch-all dispatch that guard-delegation.py blocks.
    "Agent": ("guard-delegation.py",),
    "Task":  ("guard-delegation.py",),
}
...
def _norm_role(value):
    """The engine's subagent-name normalizer, kept byte-identical in behavior to
    role-based-dispatch.py's `norm`. Both hooks must fold the same way or a
    spelling denied by one is auto-approved by the other."""
...
def rule_blocks(rule, tool, tool_input):
    ...
    if tool == "Agent":
        ...
        # boundary is the delegation contract's ALLOWED/FORBIDDEN block, per
        # dispatch, rather than a fixed roster. This branch stays so that any
        # future `Agent(...)` rule an operator adds is still honoured here.
